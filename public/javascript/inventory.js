...
addInventoryButton.addEventListener('click', () => {
  displayConfirmationModal(
    'Are you sure you want to add this inventory?',
    async () => {
...
selectDetector(
  'manage-inventory__dropdown',
  'main-inventory__update',
  'main-inventory__add'
);
...
function displayAll(inventoryList) {
  ...
  for (let i = 0; i < inventoryList.length; i++) {
    ...
    document
      .querySelector(`#edit-${inventoryList[i].warehouse_id}`)
    ...
    document
      .querySelector(`#delete-${inventoryList[i].warehouse_id}`)
      .addEventListener('click', () => {
        displayConfirmationModal(
...
function prepareEditInventoryModal(inventory) {
  ...
  document.querySelector('#inventory__address--update').value =
    inventory.address;
  ...
  document.querySelector('#submit-update-button').onclick = async () => {
    displayLoadingModel();
    inventory.warehouse_name = document.querySelector(
      '#inventory__name--update'
    ).value;
    inventory.address = document.querySelector(
      '#inventory__address--update'
    ).value;
    inventory.volume = document.querySelector(
      '#inventory__volume--update'
    ).value;
    const body = {
      user_credential: JSON.parse(sessionStorage.getItem('sqlUser')),
      query: inventory,
...
searchBySelector.addEventListener('change', () => {
